cv.hqreg <- function(X, y, ..., ncores = 1, nfolds=10, fold.id, type.measure = c("deviance", "mse", "mae"), 
  ...
  if (ncores > 1) {
    ...
    clusterExport(cluster, c("fold.id", "X", "y", "cv.args", "loss.args"), 
                  envir=environment())
    clusterCall(cluster, function() require(hqreg))
    fold.results <- parLapply(cl = cluster, X = 1:nfolds, fun = cvf, XX = X, y = y, 
                              fold.id = fold.id, cv.args = cv.args, loss.args = loss.args)
    stopCluster(cluster)
  }
  ...
  for (i in 1:nfolds) {
    if (parallel) {
      ...
      fit.i <- cvf(i, X, y, fold.id, cv.args, loss.args)
    }
...
cvf <- function(i, XX, y, fold.id, cv.args, loss.args) {
